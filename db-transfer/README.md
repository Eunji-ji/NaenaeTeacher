...
pg_restore -U postgres -d naenae_teacher --clean --if-exists --no-owner --no-privileges .\naenae_teacher_test.dump
```

Docker의 PostgreSQL 16 컨테이너로 복원하는 경우:

```powershell
docker cp .\naenae_teacher_test.dump <컨테이너명>:/tmp/naenae_teacher_test.dump
docker exec <컨테이너명> pg_restore -U naenae -d naenae_teacher --clean --if-exists --no-owner --no-privileges /tmp/naenae_teacher_test.dump
```
